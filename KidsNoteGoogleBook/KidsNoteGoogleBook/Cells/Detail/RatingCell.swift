...
class RatingCell: BaseTableViewCell {
    private var headerLabel: UILabel!
    private var arrowImageView: UIImageView!
    private var overallRatingLabel: UILabel!
    private var totalRatingCountLabel: UILabel!
    
    private var starImageViews: ContiguousArray<UIImageView> = []
    private var starLabels: ContiguousArray<UILabel> = []
    private var starProgressViews: ContiguousArray<UIProgressView> = []
    ...
    private func setupUI() {
        ...
        headerLabel = createLabel(text: "평점 및 리뷰", font: .boldSystemFont(ofSize: 16), textColor: .label)
        arrowImageView = createImageView(image: UIImage(systemName: "chevron.right"), tintColor: .selectedTab)
        overallRatingLabel = createLabel(font: .boldSystemFont(ofSize: 45), textAlignment: .center)
        totalRatingCountLabel = createLabel(font: .systemFont(ofSize: 10), textColor: .naviTint, textAlignment: .center)
        ...
        for _ in 1...5 {
            let starImageView = createImageView(image: emptyStarImage, tintColor: .selectedTab)
            ratingView.addSubview(starImageView)
            starImageViews.append(starImageView)
            
            let starLabel = createLabel(font: .systemFont(ofSize: 10))
            ratingView.addSubview(starLabel)
            starLabels.append(starLabel)
            
            let starProgressView = createProgressView(progressTintColor: .selectedTab, trackTintColor: .naviLine)
    ...
    func configure(rating: Double, ratingCount: Int) {
        ...
        var starCounts = ContiguousArray<Int>(repeating: 0, count: 5)
